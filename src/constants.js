export const SERVER_HOST = window.location.hostname;
export const QUERY_ENDPOINT = `http://${SERVER_HOST}:3001/query`;
export const COMMON_GREMLIN_ERROR = 'Invalid query. Please execute a query to get a set of vertices';
export const ACTIONS = {
  SET_HOST: 'SET_HOST',
  SET_PORT: 'SET_PORT',
  SET_QUERY_TYPE: 'SET_QUERY_TYPE',
  SET_QUERY: 'SET_QUERY',
  SET_QUERY_BUILDER_SERVICE: 'SET_QUERY_BUILDER_SERVICE',
  SET_QUERY_BUILDER_TRACING: 'SET_QUERY_BUILDER_TRACING',
  SET_QUERY_BUILDER_TRACING_STEPS: 'SET_QUERY_BUILDER_TRACING_STEPS',
  SET_QUERY_BUILDER_TRACING_DIRECTION: 'SET_QUERY_BUILDER_TRACING_DIRECTION',
  SET_ERROR: 'SET_ERROR',
  SET_NETWORK: 'SET_NETWORK',
  CLEAR_GRAPH: 'CLEAR_GRAPH',
  ADD_NODES: 'ADD_NODES',
  ADD_EDGES: 'ADD_EDGES',
  SET_SELECTED_NODE: 'SET_SELECTED_NODE',
  SET_SELECTED_EDGE: 'SET_SELECTED_EDGE',
  SET_IS_PHYSICS_ENABLED: 'SET_IS_PHYSICS_ENABLED',
  ADD_QUERY_HISTORY: 'ADD_QUERY_HISTORY',
  CLEAR_QUERY_HISTORY: 'CLEAR_QUERY_HISTORY',
  SET_NODE_LABELS: 'SET_NODE_LABELS',
  ADD_NODE_LABEL: 'ADD_NODE_LABEL',
  EDIT_NODE_LABEL: 'EDIT_NODE_LABEL',
  REMOVE_NODE_LABEL: 'REMOVE_NODE_LABEL',
  REFRESH_NODE_LABELS: 'REFRESH_NODE_LABELS',
  SET_NODE_LIMIT: 'SET_NODE_LIMIT'
};
